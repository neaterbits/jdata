...
public abstract class BaseXMLStorage implements IItemStorage, AutoCloseable {
	...
	// TODO we cannot really store this as a file in S3 since does no
	// read-after=write consistency on update
	...
	/**
	 * List all files indices in-order. These might not be monotonically
	 * increasing since files may have been deleted
	 * 
	 * @param userId
	 * @param itemId
	 * @param itemFileType
	...
	private String[] getFileNameParts(String fileName) {
		// Cannot split with String.split() since FILE_NAME_SEPARATOR might be
		// regex reserved char
	...
	protected final String allocateFileName(String userId, String itemId, ItemFileType itemFileType, String mimeType)
		...
		return String.valueOf(allocatedId) + FILE_NAME_SEPARATOR + mimeType.replace('/', '_') + FILE_NAME_SEPARATOR
				+ itemId;
	}
	...
	private final String getImageFileName(String userId, String itemId, ItemFileType itemFileType, Images images,
			int fileNo) {
	...
	protected final void writeAndCloseOutput(InputStream inputStream, OutputStream outputStream)
			throws StorageException {
	...
	protected final int addToImageList(String userId, String itemId, String thumbnailFileName, String thumbnailMimeType,
	...
	protected final void removeFromImageList(String userId, String itemId, String thumbnailFileName,
			String photoFileName) throws StorageException {

		final Images imageList = getImageList(userId, itemId);

		imageList.getImages()
				.removeIf(image -> image.getId().equals(itemId)
	...
	@Override
	public final void movePhotoAndThumbnailForItem(String userId, String itemId, int photoNo, int toIndex)
			throws StorageException {
	...
	protected abstract InputStream getImageListInputForItem(String userId, String itemId, String fileName)
			throws StorageException;

	protected abstract void storeImageListForItem(String userId, String itemId, String fileName,
			InputStream inputStream, Integer contentLength) throws StorageException;

	protected abstract ImageResult getImageFileForItem(String userId, String itemId, ItemFileType itemFileType,
	...
	private ImageResult downloadFileFromNet(String url) {
		...
		if (connection != null) {
			connection.setRequestProperty("User-Agent",
					"Mozilla/5.0 (MS-Windows; rv:60.0) Gecko/20100101 Firefox/60.0");
	...
	private ImageResult getImageForItem(String userId, String itemId, int photoNo, ItemFileType itemFileType)
			throws StorageException {
	...
	@Override
	public final ImageMetaData getThumbnailMetaDataForItem(String userId, String itemId, int photoNo)
			throws StorageException {
		...
		if (imageList == null || imageList.getImages() == null || imageList.getImages().size() <= photoNo) {
			...
			metaData = new ImageMetaData(imageData.getMimeType(), imageData.getSize(), imageData.getWidth(),
					imageData.getHeight());
		}
	...
	private static class ReadImage {
		...
		private final int numItemThumbnails;

		public ReadImage(ItemId itemId, ImageResult imageResult, int numItemThumbnails) {
	...
	public void retrieveThumbnails(ItemId[] itemIds, BiConsumer<ImageResult, ItemId> consumer) throws StorageException {
		retrieveThumbnails(itemIds, (imageResult, itemId, numThumbsForItem) -> consumer.accept(imageResult, itemId),
				false);
	}

	@Override
	public void retrieveThumbnailsWithCount(ItemId[] itemIds, OnThumbnails consumer) throws StorageException {
		retrieveThumbnails(itemIds, consumer, true);
	}

	private void retrieveThumbnails(ItemId[] itemIds, OnThumbnails consumer, boolean retrieveNumThumbnailsForItem)
			throws StorageException {
		...
		for (ItemId itemId : itemIds) {
			...
			final int numThumbsForItem;

			if (retrieveNumThumbnailsForItem) {
				numThumbsForItem = imageList != null ? imageList.getImages().size() : 0;
			} else {
				numThumbsForItem = 0;
			}
			...
			if (imageList == null) {
				...
				try {
					imageQueue.put(new ReadImage(itemId, nullImageResult(), numThumbsForItem));
				...
				if (getNumThumbnailFilesForItem(itemId.getUserId(), itemId.getItemId()) > 0) {
					...
					thumbDownloadService.submit(() -> {
						...
						try {
							final String fileName = getImageFileName(itemId.getUserId(), itemId.getItemId(),
									ItemFileType.THUMBNAIL, imageList, 0);

							try {
								image = getImageFileForItem(itemId.getUserId(), itemId.getItemId(),
										ItemFileType.THUMBNAIL, fileName);
						...
						try {
							imageQueue.put(new ReadImage(itemId, image, numThumbsForItem));
					...
					if (imageList.getImages() == null) {
						...
						thumbUrl = imageList.getImages().isEmpty() ? null : imageList.getImages().get(0).getThumbUrl();
					}
		...
		final Map<String, ReadImage> retrievedThumbs = new HashMap<>();
		...
		while (processed < itemIds.length) {
			...
			retrievedThumbs.put(read.itemId.getItemId(), read);
			...
			for (int i = nextIndexToSendOn; i < itemIds.length; ++i) {
				...
				if (retrievedThumbs.containsKey(itemId)) {
					...
					final ReadImage readImage = retrievedThumbs.get(itemId);

					consumer.onThumbnails(readImage.imageResult, itemIds[i], readImage.numItemThumbnails);
