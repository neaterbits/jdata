...
public class LuceneItemIndexTest extends TestCase {
	...
	public void testAddAndSearch() throws Exception {
		...
		try (LuceneItemIndex index = new LuceneItemIndex(new RAMDirectory())) {
			...
			// Integer range tests
			cursor = search(index, new IntegerRangeCriterium(yearAttribute, 2015, false, 2017, false));
			assertThat(cursor.getTotalMatchCount()).isEqualTo(0);
	
			cursor = search(index, new IntegerRangeCriterium(yearAttribute, 2015, true, 2017, false));
			assertThat(cursor.getTotalMatchCount()).isEqualTo(1);

			cursor = search(index, new IntegerRangeCriterium(yearAttribute, 2014, false, 2016, false));
			assertThat(cursor.getTotalMatchCount()).isEqualTo(1);

			cursor = search(index, new IntegerRangeCriterium(yearAttribute, 2013, false, 2015, false));
			assertThat(cursor.getTotalMatchCount()).isEqualTo(0);
	
			cursor = search(index, new IntegerRangeCriterium(yearAttribute, 2013, false, 2015, true));
			assertThat(cursor.getTotalMatchCount()).isEqualTo(1);

			// Decimal range tests
			cursor = search(index, new DecimalRangeCriterium(widthAttribute, new BigDecimal("32.5"), false, new BigDecimal("40.2"), false));
			assertThat(cursor.getTotalMatchCount()).isEqualTo(0);
	
			cursor = search(index, new DecimalRangeCriterium(widthAttribute, new BigDecimal("32.5"), true, new BigDecimal("40.2"), false));
			assertThat(cursor.getTotalMatchCount()).isEqualTo(1);

			cursor = search(index, new DecimalRangeCriterium(widthAttribute, new BigDecimal("30.5"), false, new BigDecimal("40.2"), false));
			assertThat(cursor.getTotalMatchCount()).isEqualTo(1);

			cursor = search(index, new DecimalRangeCriterium(widthAttribute, new BigDecimal("30.5"), false, new BigDecimal("32.5"), false));
			assertThat(cursor.getTotalMatchCount()).isEqualTo(0);
	
			cursor = search(index, new DecimalRangeCriterium(widthAttribute, new BigDecimal("30.5"), false, new BigDecimal("32.5"), true));
			assertThat(cursor.getTotalMatchCount()).isEqualTo(1);
		}
		catch (Throwable t) {
